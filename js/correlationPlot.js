class CorrelationPlot {
    constructor(svgSelector, initialData, colorFunction, topGenres) {
    ...
    _setupChartArea() {
        ...
        this.yAxisLabel = this.chartGroup.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").style("text-anchor", "middle").attr("dy", "-3em");
    ...
    resize() {
        ...
        this.xAxisLabel.attr("transform", `translate(${this.width / 2}, ${this.height + this.margin.bottom - 10})`);
        this.yAxisLabel.attr("transform", `translate(-40, ${this.height / 2}) rotate(-90)`);
    }

    update(data, attribute) {
        this.chartGroup.selectAll(".plot-element").remove();
        this.yAxisLabel.text("Average Rating");
        const yValue = d => d.rating;
        const isNumeric = ['runtime', 'year', 'rating'].includes(attribute);
    ...
    _updateBoxPlot(data, groupingAttribute, yValue) {
        ...
        const stats = Array.from(groupedData, ([key, values]) => {
            ...
            // Get representative data point for coloring and find dominant genre
        ...
        boxGroup.append("rect")
            ...
            .attr("stroke", "black")
            .style("fill", d => {
                // Use the same logic as the main getColor function
                let colorData;
                if (groupingAttribute === 'genre') {
                    colorData = { genre: d.key, type: d.representativeItem.type };
                } else if (groupingAttribute === 'type') {
                    // Use the same logic as getDominantTopGenreForType
                    const typeData = data.filter(item => item.type === d.key);
                    const genreCounts = d3.rollup(typeData, v => v.length, d => d.genre);
                    const sortedGenres = Array.from(genreCounts.entries())
                        .sort((a, b) => b[1] - a[1])
                        .map(d => d[0]);
                    const dominantTopGenre = sortedGenres.find(genre => this.topGenres.includes(genre)) || this.topGenres[0];
                    
                    colorData = { 
                        type: d.key, 
                        genre: dominantTopGenre
                    };
                } else {
                    colorData = { 
                        [groupingAttribute]: d.key, 
                        genre: d.representativeItem.genre, 
                        type: d.representativeItem.type 
                    };
                }
