class CorrelationPlot {
    constructor(svgSelector, initialData, colorFunction, topGenres, onBoxClickCallback) {
        ...
        this.onBoxClick = onBoxClickCallback;
    ...
    _setupChartArea() {
        ...
        // ... inside _setupChartArea() ...
        this.xAxisLabel = this.chartGroup.append("text")
            .attr("class", "axis-label")
            .style("text-anchor", "middle")
            .style("font-size", "35px") // Keep or increase font size
        ...
        // --- Replace the old trendToggleButton code with this ---
...
// 1. First, create the text element so we can measure it
this.trendToggleText = this.trendToggleButton.append("text")
    ...
    .style("font-size", "35px") // Your new, larger font size
    .style("font-weight", "600")
    .style("fill", "#495057")
    .text("Show Trend Line");

// 2. Get the size of the text and define padding
...
// 3. Center the text within the button's new dynamic dimensions
this.trendToggleText.attr("x", rectWidth / 2).attr("y", rectHeight / 2);

// 4. Insert the rectangle BEHIND the text with the calculated size
...
// --- Your existing .on("click", ...) and .on("mouseover", ...) handlers remain the same ---
    ...
    this.currentPath.forEach((level, i) => {
        const labelMap = {
            'type': 'Type',
            'genre': 'Genre',
            'year': 'Year',
        ...
        // Text (positioned after we calculate rect size)
        ...
        // Now, position the text perfectly in the center of the rect
        text.attr("x", buttonPadding.x)
            .attr("y", rectHeight / 2) // Position at the vertical midpoint
            .style("dominant-baseline", "middle"); // Vertically align the text to the midpoint

        // Click handler - reset to this level
        group.on("click", () => {
            this.currentPath = this.currentPath.slice(0, i + 1);
            this._renderNavigation();
            

            const currentAttribute = this.currentPath[this.currentPath.length - 1];
            this.update(this.data, currentAttribute);
        });
        ...
        if (i < this.currentPath.length - 1) {
            this.navigationGroup.append("text")
                .text("→")
                .attr("x", xPos)
                .attr("y", yPos + rectHeight / 2) // Also center the arrow with the button
                .style("dominant-baseline", "middle") // And align it
   ...
   resize() {
    ...
    // --- CHANGE THIS SECTION ---
    // Instead of setting fixed width/height, set the viewBox
    this.svg
        .attr("viewBox", `0 0 ${containerWidth} ${containerHeight}`)
        // Also remove width/height attributes in case they were set elsewhere
        .attr("width", null) 
        .attr("height", null);
    // --- END CHANGE ---
    ...
    this.yAxisLabel
        .attr("transform", `translate(-75, ${this.height / 2}) rotate(-90)`);

    this.trendToggleButton
        .attr("transform", `translate(${containerWidth - 255}, 10)`);
}
    ...
    // --- CORRECTED PATH MANAGEMENT LOGIC ---
    // This section makes the `update` method the single source of truth for the path's state.
    const attributeIndex = this.hierarchyLevels.indexOf(attribute);

    if (attributeIndex !== -1) {
        // If the attribute is a valid level in our hierarchy...

        // Check if the current path already contains this attribute.
        const existingIndexInPath = this.currentPath.indexOf(attribute);

        if (existingIndexInPath !== -1) {
            // If it exists (e.g., after a breadcrumb click), truncate the path to this point.
            this.currentPath = this.currentPath.slice(0, existingIndexInPath + 1);
        } else {
            // If it's a new drill-down, ensure the path is consistent up to the previous level, then add the new one.
            this.currentPath = this.currentPath.filter(attr =>
                this.hierarchyLevels.indexOf(attr) < attributeIndex
            );
            this.currentPath.push(attribute);
        }
    }
    // --- END CORRECTION ---

    // Update navigation based on the now-correct path
    this._renderNavigation();
    ...
    // --- ADD THIS BLOCK to ensure a tick at the end of the x-axis ---
    const xDomain = this.xScale.domain();
    const xTicks = this.xScale.ticks();

    // If the last default tick is less than the domain's max, add the max value.
    ...
    const stats = Array.from(groupedData, ([key, values]) => {
        ...
        return { 
            ...
            dominantGenre, typeDistribution, values
        };
    });
    ...
    this.xAxisGroup.selectAll("text")
        .style("font-size", "30px").style("fill", "600").style("fill", "#333");
    this.yAxisGroup.selectAll("text")
        .style("font-size", "30px").style("fill", "#333");
    ...
    const boxGroup = this.chartGroup.selectAll(".box-group")
        ...
        .on("click", (event, d) => {
            ...
            if (this.onBoxClick) {
                const currentLevelIndex = this.hierarchyLevels.indexOf(groupingAttribute);
                const depth = currentLevelIndex + 1;
                this.onBoxClick([d.key], depth);
            } else {
                console.error('onBoxClick callback is not defined');
            }
        });
    ...
    boxGroup.append("rect")
        ...
        .style("fill", d => this.color({ genre: d.dominantGenre }))
    ...
    _updateToggleButton() {
        this.trendToggleText
            .text(this.showTrendLine ? "Hide Trend Line" : "Show Trend Line");
            
        this.trendToggleButton.select("rect")
            .style("fill", this.showTrendLine ? "#d4edda" : "#f8f9fa")
            .style("stroke", this.showTrendLine ? "#c3e6cb" : "#dee2e6");
            
        this.trendToggleText
            .style("fill", this.showTrendLine ? "#155724" : "#495057");
    }

    _updateTrendLine() {
        this.chartGroup.selectAll(".trend-line").remove();

        if (!this.showTrendLine || !this.currentData || !this.xScale || !this.yScale) {
            return;
        }
        ...
        this.chartGroup.append("line")
            ...
            .attr("x1", this.xScale(lineData[0].x))
            .attr("y1", this.yScale(lineData[0].y))
            .attr("x2", this.xScale(lineData[1].x))
            .attr("y2", this.yScale(lineData[1].y))
            .style("stroke", "#e74c3c")
    ...
    _calculateLinearRegression(data, xValue, yValue) {
        const validData = data.filter(d => 
            ...
            isFinite(xValue(d)) && isFinite(yValue(d))
        );
